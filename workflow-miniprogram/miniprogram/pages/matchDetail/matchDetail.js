...
Page({
  ...
  onLoad: async function(options) {
    let res = await util.request(`/activity/${options.id}`, 'GET')
    console.log(res)
    var that = this
    wx.request({
      url:
      ...
      success: function(res) {
        ...
        // 时间格式化
        res.data.data.activitySignUpDeadline = util.formatTime(
          new Date(res.data.data.activitySignUpDeadline)
        )
        res.data.data.activityTime = util.formatTime(
          new Date(res.data.data.activityTime)
        )
        that.setData({
          match: res.data.data
        })
